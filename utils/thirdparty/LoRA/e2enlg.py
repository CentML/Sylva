...
class FT_Dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        pad_targets = (
            [0 for i in range(0, self.prefix_len)]
            + context
            + [0 for i in range(0, self.infix_len)]
            + completion
        )
        ...
        output["lm_labels"] = torch.tensor(_target, dtype=torch.long)
        output["lm_mask"] = torch.tensor(_msk, dtype=torch.float)
        return output
