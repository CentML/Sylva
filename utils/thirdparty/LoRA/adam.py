# Modifications made by CentML Inc. is licensed under Apache License, Version 2.0.
...
# ===========================================================================================
#  Below is the copyright header in the original code base.
#  ------------------------------------------------------------------------------------------
#  Copyright (c) Microsoft Corporation. All rights reserved.
#  Licensed under the MIT License (MIT). See LICENSE in the repo root for license information.
#  ------------------------------------------------------------------------------------------
#  Source: https://github.com/microsoft/LoRA/tree/main
...
from sylva.optimizer import batch_sparse_optimize
...
def add_optimizer_params(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--correct_bias", action="store_true", help="correct adam bias term"
    )
    parser.add_argument("--adam_epislon", default=1e-6, type=float, help="adam epsilon")
    parser.add_argument(
        "--no_decay_bias", action="store_true", help="no weight decay on bias weigh"
    )
    parser.add_argument("--adam_beta1", default=0.9, type=float, help="adam beta1 term")
    parser.add_argument(
        "--adam_beta2", default=0.98, type=float, help="adam beta2 term"
    )
    ...
    parser.add_argument(
        "--max_epoch", type=int, default=None, help="max epoch of training"
    )
...
class AdamW(Optimizer):
    ...
    def step(self, closure=None):
        ...
        if closure is not None:
            model, scope, block_size, m_masks = closure()
        for group in self.param_groups:
            for p in group["params"]:
                if p.grad is None:
                    continue
        ...
        batch_sparse_optimize(self, model, adam_buf, scope, block_size, m_masks)
        return loss
...
class CyclicScheduler(_LRScheduler):
    ...
    def get_lr(self):
        for _i in range(0, len(self.interval_steps) - 1):
            if (
                self.last_epoch >= self.interval_steps[_i]
                and self.last_epoch < self.interval_steps[_i + 1]
            ):
...
def create_grouped_parameters(model, no_decay_bias, lr, weight_decay):  # args):
...
def create_optimizer_scheduler(optimizer, args, max_scheduler_steps=60000):
    if args.scheduler == "cosine":
        scheduler = CosineAnnealingWarmupRestarts(
            ...
            max_steps=max_scheduler_steps,
            alpha=0,
        )
    elif args.scheduler == "linear":
        scheduler = get_linear_schedule_with_warmup(
            optimizer, args.num_warmup_steps, max_scheduler_steps, last_epoch=-1
        )
        ...
        max_scheduler_steps = args.i_steps[-1]
        print("max_step is rest to", max_scheduler_steps)
